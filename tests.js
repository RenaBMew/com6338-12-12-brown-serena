    ...
    describe('Word Class', () => {
      describe('constructor', () => {
        it('Should exist and be a Class', () => {
          ...
          expect(typeof Word).to.eq('function')
        })
        it('Should create word objects', () => {
          const word = new Word('banana')
          expect(word instanceof Word).to.be.true
        })
        it('Should create word objects with word property set to the desired word', () => {
          const word = new Word('banana')
          expect(word.word).to.eq('banana')
        })
        it('Should create word objects with displayWord property that is letters of word replaced with underscores. (dog => ___)', () => {
          const word = new Word('cat')
          expect(word.displayWord.length).to.eq(3)
          expect(word.displayWord).to.eq('___')
        })
        it('Should create word objects with remainingGuesses property set to 10', () => {
          const word = new Word('banana')
          expect(word.remainingGuesses).to.eq(10)
        })
        it('Should create word objects with correctLetters property set to an empty array', () => {
          const word = new Word('banana')
          expect(Array.isArray(word.correctLetters)).to.be.true
        })
        it('Should create word objects with incorrectLetters property set to an empty array', () => {
          const word = new Word('banana')
          expect(Array.isArray(word.incorrectLetters)).to.be.true
        })
      })
      describe('guessLetter method', () => {
        it('Should exist and be a method of the class', () => {
          expect(Word.prototype.guessLetter).to.exist
          expect(typeof Word.prototype.guessLetter).to.eq('function')
        })
        it('Should update displayWord with correctly guessed letters', () => {
          const word = new Word('banana')
          expect(word.displayWord).to.eq('______')
          word.guessLetter('b')
          expect(word.displayWord).to.eq('b_____')
        })
        it('Should NOT update displayWord with incorrectly guessed letters', () => {
          const word = new Word('banana')
          expect(word.displayWord).to.eq('______')
          word.guessLetter('z')
          expect(word.displayWord).to.eq('______')
        })
        it('Should decrement remainingGuesses when incorrectly guessing a letter', () => {
          const word = new Word('banana')
          expect(word.remainingGuesses).to.eq(10)
          word.guessLetter('z')
          expect(word.remainingGuesses).to.eq(9)
        })
        it('Should NOT decrement remainingGuesses when correctly guessing a letter', () => {
          const word = new Word('banana')
          expect(word.remainingGuesses).to.eq(10)
          word.guessLetter('b')
          expect(word.remainingGuesses).to.eq(10)
        })
        it('Should add incorrect letter to incorrectLetters property, not correctLetters property', () => {
          const word = new Word('banana')
          expect(word.correctLetters).to.be.empty
          expect(word.incorrectLetters).to.be.empty
          word.guessLetter('z')
          expect(word.correctLetters).to.be.empty
          expect(word.incorrectLetters).to.contain('z')
        })
        it('Should add correct letter to correctLetters property, not incorrectLetters property', () => {
          const word = new Word('banana')
          expect(word.correctLetters).to.be.empty
          expect(word.incorrectLetters).to.be.empty
          word.guessLetter('b')
          expect(word.correctLetters).to.contain('b')
          expect(word.incorrectLetters).to.empty
        })
      })
      describe('updateScreen method', () => {
        let word
        const wordToGuess = document.getElementById('word-to-guess')
        const remainingGuessDisplay = document.getElementById('remaining-guesses')
        const incorrectLettersDisplay = document.getElementById('incorrect-letters')
        before(() => {
          wordToGuess._textContent = wordToGuess.textContent
          remainingGuessDisplay._textContent = remainingGuessDisplay.textContent
          incorrectLettersDisplay._textContent = incorrectLettersDisplay.textContent
        })
        beforeEach(() => {
        ...
        after(() => {
          wordToGuess.textContent = wordToGuess._textContent
          remainingGuessDisplay.textContent = remainingGuessDisplay._textContent
          incorrectLettersDisplay.textContent = incorrectLettersDisplay._textContent
        })
        it('Should exist and be a method of the class', () => {
          expect(Word.prototype.updateScreen).to.exist
          expect(typeof Word.prototype.updateScreen).to.eq('function')
        })
        it('Should update #word-to-guess with displayWord', () => {
          expect(wordToGuess.textContent).to.eq("")
          word.displayWord = 'Voltron'
          word.updateScreen()
          expect(wordToGuess.textContent).to.eq('Voltron')
        })
        it('Should update #remaining-guesses with remainingGuesses', () => {
          expect(remainingGuessDisplay.textContent).to.eq("")
          word.remainingGuesses = 99
          word.updateScreen()
          expect(remainingGuessDisplay.textContent).to.eq('99')
        })
        it('Should update #incorrect-letters with incorrectLetters', () => {
          expect(incorrectLettersDisplay.textContent).to.eq("")
          word.incorrectLetters = ['x', 'y', 'z']
          word.updateScreen()
          expect(incorrectLettersDisplay.textContent.replaceAll(/\s/g, "")).to.eq('x,y,z')
        })
      })
      describe('isGameOver method', () => {
        let word
        beforeEach(() => word = new Word('banana'))
        it('Should exist and be a method of the class', () => {
          expect(Word.prototype.isGameOver).to.exist
          expect(typeof Word.prototype.isGameOver).to.eq('function')
        })
        it('Should return false if displayWord !== word and remainingGuesses > 0', () => {
          word.remainingGuesses = 10
          word.displayWord = 'voltron'
          word.word = 'thundercats'
          expect(word.isGameOver()).to.be.false
        })
        it('Should return true if displayWord === word and remainingGuesses > 0', () => {
          word.remainingGuesses = 10
          word.displayWord = 'voltron'
          word.word = 'voltron'
          expect(word.isGameOver()).to.be.true
        })
        it('Should return true if displayWord !== word and remainingGuesses <= 0', () => {
          word.remainingGuesses = 0
          word.displayWord = 'voltron'
          word.word = 'thundercats'
          expect(word.isGameOver()).to.be.true
        })
      ...
      describe('getWinOrLoss method', () => {
        let word
        beforeEach(() => word = new Word('banana'))
        it('Should exist and be a method of the class', () => {
          expect(Word.prototype.getWinOrLoss).to.exist
          expect(typeof Word.prototype.getWinOrLoss).to.eq('function')
        })
        it('Should return null if game is not over', () => {
          word.remainingGuesses = 99
          word.displayWord = 'voltron'
          word.word = 'thundercats'
          expect(word.getWinOrLoss()).to.be.null
        })
        it('Should return "win" if displayWord === word and remainingGuesses > 0', () => {
          word.remainingGuesses = 99
          word.displayWord = 'voltron'
          word.word = 'voltron'
          expect(word.getWinOrLoss()).to.eq('win')
        })
        it('Should return "loss" if remainingGuesses <= 0 and displayWord !== word', () => {
          word.remainingGuesses = 0
          word.displayWord = 'voltron'
          word.word = 'thundercats'
          expect(word.getWinOrLoss()).to.eq('loss')
        })
      })
    })
